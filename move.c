...
void 	vect_normin(t_mlx *d)
{
	...
	while (k++ < d->el_num)
	{
		res = sqrt(d->coords[k].x * d->coords[k].x + d->coords[k].y * d->coords[k].y + d->coords[k].z * d->coords[k].z);
		d->coords[k].x /= res;
		d->coords[k].y /= res;
		d->coords[k].z /= res;
	}
...
void	mtr_mult(t_mlx *d, t_mtr *mtr)
{
	...
	while (k++ < d->el_num)
	{
		...
		d->coords[k].x = d->coords[k].x * mtr->m1 + d->coords[k].y * mtr->m2 + 
		d->coords[k].z * mtr->m3 + d->coords[k].w * mtr->m4;
		d->coords[k].y = d->coords[k].x * mtr->m5 + d->coords[k].y * mtr->m6 + 
		d->coords[k].z * mtr->m7 + d->coords[k].w * mtr->m8;;
		d->coords[k].z = d->coords[k].x * mtr->m9 + d->coords[k].y * mtr->m10 + 
		d->coords[k].z * mtr->m11 + d->coords[k].w * mtr->m12;
